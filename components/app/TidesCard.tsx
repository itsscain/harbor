  ...
  return (
    <Card className="border-accent/30 bg-accent/[0.04]">
      ...
      <button
        onClick={gen}
        disabled={loading}
        className="mt-3 inline-flex items-center gap-1.5 rounded-xl border border-accent/40 px-3.5 py-2 text-sm font-semibold text-accent transition hover:bg-accent/10 disabled:opacity-60"
      >
      ...
      {insight && (
        <div className="mt-3 space-y-3">
          <p className="text-pretty text-sm text-fg">{insight.summary}</p>
          ...
          {insight.enough_data && insight.suggestion && (
            <p className="text-pretty rounded-xl bg-accent/10 px-3.5 py-2.5 text-sm text-fg">💡 Try: {insight.suggestion}</p>
          )}
        </div>
      )}
    </Card>
