-- Edge Cases §1.1 — the idempotent point ledger. The push RPC already dedupes on
-- client_op_id (on conflict do nothing + gated increment); the only hole was that
-- each device minted a RANDOM op_id, so two offline devices completing the same step
-- on the same day awarded twice (test K1). Fix: completion op_ids become DETERMINISTIC
-- ("<child>:<source>:<id>:<serviceDay>") so the same step/chore/day collapses to ONE
-- award across devices and re-completes. Switch client_op_id to text to carry the key.
-- (Recreated from the live function — preserves the chore-rotation crediting from 0025.)
alter table public.reward_log alter column client_op_id type text using client_op_id::text;
...
begin
  ...
  loop
    ...
    -- Accept the anchor child OR any current rotation member (the wall credits
    -- whoever the chore lands on this week).
