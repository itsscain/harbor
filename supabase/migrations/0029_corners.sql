-- Calm Corner: a tracked, supportive timeout. A parent starts one (with a reason);
-- it runs an age-based timer, shows a calm plan + encouragement on the wall, and
-- afterward an AI reflection helps the family understand patterns and do better.
-- Framed as co-regulation / a reset — never shaming.
create table if not exists public.corners (
  ...
  plan jsonb,        -- { steps: text[], reminder: text, encouragement: text }
  report text,       -- AI after-reflection, parent-facing
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  deleted_at timestamptz
);

create index if not exists corners_child_idx on public.corners(child_id) where deleted_at is null;
-- At most one active corner per child at a time.
create unique index if not exists corners_one_active
  on public.corners(child_id) where status = 'active' and deleted_at is null;

alter table public.corners enable row level security;

create policy corners_all on public.corners for all
...
create trigger set_updated_at_corners
  before update on public.corners
  for each row execute function public.set_updated_at();

-- Rebuild kiosk_snapshot to flow still-running corners to the wall.
-- (Same body as 0023 with the 'corners' key added after 'groundings'. On a full
--  pull we send only active corners whose timer hasn't elapsed, so the wall never
--  shows a stale timeout; deltas ride updated_at so a closed corner syncs away.)
