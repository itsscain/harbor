    ...
    <>
        ...
        {(sharedForChild ?? []).length > 0 && (
          <Card className="border-dashed">
            <p className="text-sm text-fg-muted">
              <span className="font-semibold text-fg">{child.name} also does:</span>{" "}
              {(sharedForChild ?? []).map((r) => r.name).join(", ")} — shared family{" "}
              {(sharedForChild ?? []).length === 1 ? "routine" : "routines"}, managed in the{" "}
              <Link href="/app/schedule" className="font-semibold text-accent underline">
                Family Schedule
              </Link>
              .
            </p>
      ...
      <Card className="mb-4 p-0">
        <Disclosure bodyClassName="px-5 pb-5" summary={
          <div>
            <span className="text-title text-fg">Add a routine</span>
            <span className="mt-0.5 block text-sm text-fg-muted">Start from a template or build your own</span>
          </div>
        }>
        <p className="mt-1 text-sm text-fg-muted">Quick start by grade — adds Morning, After school &amp; Bedtime, tuned for their age:</p>
        ...
        <p className="mt-4 text-sm text-fg-muted">Or start from a template — pick one, then tweak. Designed around real needs:</p>
        <div className="mt-2.5">
          <TemplateGallery templates={routineTemplates ?? []} childId={child.id} />
        </div>
        <p className="mt-4 text-sm font-semibold text-fg">…or build your own:</p>
      ...
      <Card className="mb-4 p-0">
        <Disclosure bodyClassName="px-5 pb-5" summary={
          <div>
            <span className="text-title text-fg">Chores</span>
            <span className="mt-0.5 block text-sm text-fg-muted">Tasks to earn stars on the wall</span>
          </div>
        }>
        ...
        {(chores ?? []).length > 0 ? (
          <div className="mt-3 space-y-2">
            {(chores ?? []).map((ch) => {
              ...
              return (
                <div key={ch.id} className="rounded-xl border border-line p-3">
                  ...
                  <form key={ch.updated_at} action={updateChore.bind(null, ch.id, child.id)} className="space-y-2.5">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d, i) => (
                        <label key={i} className="cursor-pointer">
                          ...
                          <span className="flex h-8 w-10 items-center justify-center rounded-lg border border-line text-xs font-semibold text-fg-muted transition peer-checked:border-accent peer-checked:bg-accent/10 peer-checked:text-accent">
                            {d.slice(0, 2)}
                          </span>
                        </label>
                      ))}
                    ...
                    <label className="flex items-center gap-2 text-xs font-medium text-fg">
                      <input type="checkbox" name="approval" defaultChecked={ch.requires_approval ?? false} className="h-4 w-4 rounded border-line-strong text-accent focus:ring-accent" />
          ...
          <p className="mt-3 text-sm text-fg-muted">No chores yet — add the first one below.</p>
        )}
        <form action={createChore.bind(null, child.id)} className="mt-3 space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d, i) => (
                <label key={i} className="cursor-pointer">
                  ...
                  <span className="flex h-9 w-12 items-center justify-center rounded-lg border border-line text-sm font-semibold text-fg-muted transition peer-checked:border-accent peer-checked:bg-accent/10 peer-checked:text-accent">
                    {d}
                  </span>
                </label>
              ))}
          ...
          <label className="flex items-center gap-2.5 rounded-xl border border-line px-3.5 py-3 text-sm font-medium text-fg">
            <input type="checkbox" name="rotate" className="h-4 w-4 rounded border-line-strong text-accent focus:ring-accent" />
            <span className="flex items-center gap-1.5">
              <RefreshCw className="h-4 w-4 text-accent" /> Rotate between all the kids each week
            </span>
          </label>
          <label className="flex items-center gap-2.5 rounded-xl border border-line px-3.5 py-3 text-sm font-medium text-fg">
            <input type="checkbox" name="approval" className="h-4 w-4 rounded border-line-strong text-accent focus:ring-accent" />
            <span className="flex items-center gap-1.5">
              <ShieldCheck className="h-4 w-4 text-accent" /> Needs a grown-up&apos;s OK to check off
            </span>
          </label>
      ...
      <Card className="mb-4 p-0">
        <Disclosure bodyClassName="px-5 pb-5" summary={
          <div>
            <span className="text-title text-fg">Profile</span>
            <span className="mt-0.5 block text-sm text-fg-muted">Photo, name, birthday, color</span>
          </div>
        }>
      ...
      <Card className="mb-4 p-0">
        <Disclosure bodyClassName="px-5 pb-5" summary={
          <div>
            <span className="text-title text-fg">Accessibility & wall</span>
            <span className="mt-0.5 block text-sm text-fg-muted">Read-aloud, sound, sensory, theme, bedtime</span>
          </div>
        }>
        <p className="text-sm text-fg-muted">How {child.name}&apos;s wall reads, sounds, and feels.</p>
      ...
      {(cs.voiceChat === true || (voiceLog ?? []).length > 0) && (
        <Card className="mb-4 p-0">
          <Disclosure
            ...
            summary={
              <div>
                <span className="text-title text-fg">Voice activity</span>
          ...
          >
            ...
            {(voiceLog ?? []).length === 0 ? (
              <p className="mt-3 text-sm text-fg-muted">No conversations yet.</p>
            ) : (
              <ul className="mt-3 space-y-2.5">
                {(voiceLog ?? []).map((v, i) => (
                  <li
                    key={i}
                    className={"rounded-xl border p-3 " + (v.distress ? "border-error/30 bg-error/10" : "border-line")}
                  >
                    {v.distress && (
                      <p className="mb-1 text-xs font-bold uppercase tracking-wide text-error">⚠️ Harbor flagged this for you</p>
                    )}
                    <p className="text-sm text-fg">&ldquo;{v.transcript}&rdquo;</p>
                    <p className="mt-1 text-sm text-fg-muted">Harbor: {v.reply}</p>
                    <p className="mt-1 text-xs text-fg-subtle">{formatInTz(new Date(v.created_at), DEFAULT_TZ, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</p>
                  </li>
                ))}
              </ul>
            )}
      ...
      <Card className="border-error/30">
        <h3 className="text-title text-error">Danger zone</h3>
        <div className="mt-3 space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="font-semibold text-fg">Hide from the wall</p>
              <p className="text-sm text-fg-muted">Removes {child.name} from the wall and app. Their data is kept.</p>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-error/20 pt-3">
            <div>
              <p className="font-semibold text-fg">Delete permanently</p>
              <p className="text-sm text-fg-muted">Erases {child.name} and all their routines, points, and history. Can&apos;t be undone.</p>
            </div>
